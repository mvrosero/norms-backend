    ...
    try {
        ...
        const [rows] = await db.promise().query(`
            SELECT 
                ...
                IFNULL(user_history.new_status, '') AS new_status,
                IFNULL(user_history.old_batch, '') AS old_batch,
                IFNULL(user_history.new_batch, '') AS new_batch,
                IFNULL(old_role.role_name, '') AS old_role_name,
                IFNULL(new_role.role_name, '') AS new_role_name,
                user_history.updated_by, 
                CONCAT(updated_by.first_name, ' ', updated_by.last_name, ' ', updated_by.suffix) AS updated_by
            FROM 
                user_history
