...
router.post('/registerUser', async (req, res) => {
    try {
        ...
        if (role_id === 6) { /* student registration */
            ...
            const hashedPassword = await bcrypt.hash(password, 10);
            const insertEmployeeQuery = 'INSERT INTO user (employee_idnumber, fullname, birthdate, email, password, year_level, profile_photo_filename, role_id, department_id, program_id) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';
            await db.promise().execute(insertEmployeeQuery, [employee_idnumber, fullname, birthdate, email, hashedPassword, year_level, profile_photo_filename, role_id, department_id, program_id]);
        }
