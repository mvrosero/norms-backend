...
router.post('/registerUser', async (req, res) => {
    try {
        ...
        if (role_id === 6) { /* student registration */
            ...
            const { employee_idnumber, fullname, birthdate, email, password,  profile_photo_filename,  } = req.body;
            const hashedPassword = await bcrypt.hash(password, 10);
            const insertEmployeeQuery = 'INSERT INTO user (employee_idnumber, fullname, birthdate, email, password, profile_photo_filename, role_id) VALUES (?, ?, ?, ?, ?, ?, ?)';
            await db.promise().execute(insertEmployeeQuery, [employee_idnumber, fullname, birthdate, email, hashedPassword, profile_photo_filename, role_id]);
        }
